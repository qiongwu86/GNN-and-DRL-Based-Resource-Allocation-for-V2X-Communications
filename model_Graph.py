...
class GraphModel(keras.Model):
    ...
    def call(self, inputs):
        ...
        if self.depth == 1:
            ...
            node_fea = tf.nn.embedding_lookup(features, batch_nodes)
            node_fea = tf.cast(node_fea, tf.float32)
            neigh_1_fea = tf.nn.embedding_lookup(features, s1_neighs)
            ...
            neigh_1_fea = tf.cast(neigh_1_fea, tf.float32)
            ...
            weighted_features_s1 = tf.cast(weighted_features_s1, tf.float32)
            ...
            weights_expanded = tf.cast(weights_expanded, tf.float32)
            neigh_2_fea = tf.cast(neigh_2_fea, tf.float32)
            weighted_features_s2 = tf.multiply(neigh_2_fea, weights_expanded)
            weighted_features_s2 = tf.cast(weighted_features_s2, tf.float32)
